...
class CommentsList extends Component {
    ...
    updateNewCommentInState = (prevProps) => {
        if (prevProps.comments !== this.props.comments) {
            ...
            // update the state with the new comments that has being passed from App.js after socket.io update
            this.setState({
                comments:this.props.comments,
            } , () => {
                this.orderComments(this.state.comments, 0) // assigning the commments to state
    ...
    orderComments = (
        ...
        comments.map((comment) => { // mapping the comments
            this.holder.push({ // pushing to holder comment object with the comment data
...
export default connect(mapStateToProps, {orderCommentsToStore})(CommentsList);