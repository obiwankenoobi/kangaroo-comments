    ...
    render() {
        ...
        return (
            ...
            <div style={{float:'left'}}>
                <Input placeholder='whats your name?' error={this.state.noEnoughChars ? true : false} name='userName' value={this.state.userName} onChange={this.handleInput} />

                <MaterialBtn style={{marginLeft:'3px', marginBottom:'5px'}} onClick={() => this.isEnoughChars(this.state.userName, () => this.props.nameLoginHandler(this.state.userName))} color='primary'>Join</MaterialBtn>      
            </div>
            <br/>
            { this.state.noEnoughChars ? <label style={{color:'red', float:'left', clear:'left'}}>min 5 and max 15 chars</label> : null }
            </div>
...
export default connect(mapStateToProps, {nameLoginHandler})(NameLogin);